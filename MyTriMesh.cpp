...
// find all holes
vector<TriMesh::VertexHandle> Original_boundary_vertex_Sort(TriMesh& mesh, vector<TriMesh::VertexHandle>& vhandle_boundary)
{
...
void Calc_geodesicDistance(TriMesh& mesh, UnitHole& hole, vector<vector<double>>& geodesicDistance)
{
	...
	for (int i = 0; i < hole.hole_original_vertices.size(); i++)
	{
		...
		for (auto it = mesh.vf_begin(hole.hole_original_vertices[i].first);
			it != mesh.vf_end(hole.hole_original_vertices[i].first); it++)
		{
			ft_tmp.insert(it->idx());	// Face가 중복으로 들어갈 수도 있으니까 set에 대입
		}
	}

	vector<int> ft_vector(ft_tmp.begin(), ft_tmp.end());
	...
	//igl::list_to_matrix(ft_vector, FT);	// FT 에 boundary Faces index 입력
...
#pragma region geodesic, controlVertex Settings
	...
	for (int i = 0; i < hole.hole_original_vertices.size(); i++)	// OriginalVertex를 controlvertex로 해줬음 -> 근데 Deformation에서 어차피 새로운 Vertex만 움직이기 때문에 사실상 의미 x
	{
...
#pragma endregion
	...
	for (int i = 0; i < hole.new_Faces.size(); i++) // new Verteices
	{
		...
		for (int j = 0; j < hole.hole_original_vertices.size(); j++)
		{
			TriMesh::Normal vertexNormal = mesh.normal(hole.hole_original_vertices[j].first);


			x_n_average += (double)(vertexNormal[0] / geodesicDistance[hole.new_Faces[i].idx()][j]);
			y_n_average += (double)(vertexNormal[1] / geodesicDistance[hole.new_Faces[i].idx()][j]);
			z_n_average += (double)(vertexNormal[2] / geodesicDistance[hole.new_Faces[i].idx()][j]);
		}

		x_n_average /= hole.hole_original_vertices.size();
		y_n_average /= hole.hole_original_vertices.size();
		z_n_average /= hole.hole_original_vertices.size();
		...
		*/
		...
		left_.m_x = result_vh2[0];
		left_.m_y = result_vh2[1];
		left_.m_z = result_vh2[2];

		right_.m_x = result_vh1[0];
		right_.m_y = result_vh1[1];
		right_.m_z = result_vh1[2];
