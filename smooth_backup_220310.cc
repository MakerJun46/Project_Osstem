...
void DrawMesh(TriangleMesh &mesh, vector<allquadrics::Quadric> qfits)
{
	Viewport viewport;
	...
	while (true) {
		...
		if (logics > 0) {
			...
			display(viewport, mesh, meshes[showQuadricFit]);
		}
		else {
...
vector<UnitHole> find_All_UnitHoles(MyMesh& mesh, bool useQuadrics, allquadrics::Quadric qfit)
{
	...
	while (!vhandle_boundary.empty())
	{
		...
		for (MyMesh::VertexHandle vh : vhandle_tmp)
		{
		...
		std::cout << "찾은 구멍 index : " << hole_number << endl;
...
int main(int argc, char* argv[])
{
	...
	char* input = "../Smoothing/bunny_twoHoles.obj";	// file input path
	...
	if (!opt.check(OpenMesh::IO::Options::VertexNormal) &&
		mesh_findBoundary.has_face_normals() && mesh_findBoundary.has_vertex_normals())
	{
		// let the mesh update the normals
		mesh_findBoundary.update_normals();
	}
	...
	while (true)
	{
		...
		cout << endl << "quadrics 선택 (-1 : quadrics 미사용, 0 : general, 1 : rotationally symmetric, 2 : plane, 3 : sphere) : ";
