...
bool vertexCmp(MyMesh mesh, MyMesh::VertexHandle a, MyMesh::VertexHandle b)
{
	MyMesh::Point a_p = mesh.point(a);
	MyMesh::Point b_p = mesh.point(b);

	double sum_a = a_p[0] + a_p[1] + a_p[2];
	double sum_b = b_p[0] + b_p[1] + b_p[2];
...
double angle_func(MyMesh& mesh, MyMesh::VertexHandle vh)
{
...
void angle_Update(MyMesh& mesh, vector<pair<MyMesh::VertexHandle, MyMesh::Point>>& holes, vector<pair<int, double>>& index_angles)
{
	...
	{
		index_angles.push_back(make_pair(i, angle_func(mesh, holes[i].first)));
	}
	...
	for (int i = 0; i < holes.size(); i++)
	{
		...
		for (int j = 0; j < holes.size(); j++)
		{
				...
				&& hh1 == MyMesh::InvalidHalfedgeHandle && hh2 == MyMesh::InvalidHalfedgeHandle && !isClose_OtherVertex_Exist(holes, i, j))
			{
				...
				fill_triangles_afterMerge(mesh, holes[j].first);
...
int main(int argc, char* argv[])
{
	...
	if (!OpenMesh::IO::read_mesh(mesh_findBoundary, "../Smoothing/bunnyhole.off"))
	{
	...
	mesh_findBoundary.request_face_normals();
