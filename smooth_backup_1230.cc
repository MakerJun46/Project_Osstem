		...
		if (vhandle_boundary.size() == 1)
		{
			...
			if (vertex_tmp.idx() == next_v.idx())
			{
				...
				if (next_v.idx() == vertex_startPoint.idx())
				{
					...
					cout << "찾은 holes 개수 : " << unit_holes.size() << "개" << endl;
				}
				else
				{
	...
	map<int, bool> isOriginal_Vertex, double edge_length_average)
{
	...
	while (!holes.empty())
	{
		...
		else // 135 < 사이각 인 경우 -> 점 2개 추가, 점 1개 제외, 삼각형 3개 추가
		{
			...
			/*
			vec3 a1 = v1 + center_coord;	// v1과 center coord의 중점
			vec3 b1 = v2 + center_coord;	// v2와 center coord의 중점
...
int main(int argc, char* argv[])
{
	...
	map<int, bool> isOriginal_Vertex;	// 최초의 vertex인지 저장
	...
	mesh_findBoundary.request_face_normals(); // 필요한 속성 request


	set_isOriginal_Vertex_array(mesh_findBoundary, isOriginal_Vertex); // 최초의 vertex들의 index 저장
	length_average = edge_length_average(mesh_findBoundary); // edge의 평균 길이 계산

	boundary_Update(mesh_findBoundary, holes);  // boundary 찾고 sort
	angle_Update(mesh_findBoundary, holes, index_angles); // 사이각 계산 및 sort
	fill_holes(mesh_findBoundary, holes, index_angles, isOriginal_Vertex, length_average); // 구멍 메우기 시작

	save(mesh_findBoundary, "result"); // 결과 저장
